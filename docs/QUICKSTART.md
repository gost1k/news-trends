...
npm run db:seed
```
Перед заливкой очищает прежние демо-таблицы, затем создаёт ~100 записей в основных сущностях и «горячие» тренды с десятками статей. Пароль для всех демо-пользователей: `demo123`.
