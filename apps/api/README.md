...
npm run db:seed      # Заполнить БД демо-данными (источники, статьи, тренды, пользователь)
npm run db:studio    # GUI для просмотра данных
```

## Демо-данные

Скрипт `prisma/seed.ts` перед заливкой **удаляет** прежние демо-данные (статьи, тренды, теги, локации, источники, связи; пользователей `*@newsmap.local`), затем создаёт:

- **~100** источников, статей, локаций, тегов
- **~200** связей `article_locations` и `article_tags` (по 2 на статью)
- **12 трендов** в формате «горячих» тем (например, эскалация Израиль — Иран, выборы в США), у каждого **10–25** статей (`article_count` соответствует)

```bash
npm run db:seed
```

**Пользователи (пароль `demo123`):** `demo@newsmap.local`, `admin@newsmap.local`, `user1@newsmap.local` … `user8@newsmap.local`

Не запускай на продакшен-БД без осознанной необходимости (полная очистка демо-таблиц).
