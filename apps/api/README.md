...
Скрипт `prisma/seed.ts` заполняет БД реалистичными данными: источники новостей (РИА, ТАСС, Lenta, Meduza), статьи, локации, тренды и демо-пользователя.
...
После выполнения:
- **Тренды:** 4 тренда с привязанными статьями
- **Статьи:** 8 новостей с AI-метаданными
- **Демо-пользователь:** `demo@newsmap.local` / `demo123`

Скрипт идемпотентен: повторный запуск обновляет существующие записи, не создавая дубликатов.
